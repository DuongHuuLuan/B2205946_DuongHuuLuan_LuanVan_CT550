  ...
  <div class="row g-3">
    <div class="col-12">
      <div class="d-flex align-items-start align-items-md-center justify-content-between gap-2 flex-column flex-md-row">
        <div>
          <h4 class="mb-1">Tạo phương thức thanh toán</h4>
          <div class="small opacity-75">Nhập tên phương thức và tạo mới</div>
        </div>
          ...
          <Form :validation-schema="schema" @submit="onSubmit" v-slot="{ isSubmitting, resetForm }">
            <div class="mb-3">
              ...
              <Field name="name" v-slot="{ field, meta }">
                <input v-bind="field" type="text" class="form-control bg-transparent"
                  :class="{ 'is-invalid': meta.touched && !meta.valid }" placeholder="VD: COD" />
              </Field>
            ...
            <div class="d-flex gap-2">
              <button class="btn btn-accent" type="submit" :disabled="isSubmitting">
                ...
                {{ isSubmitting ? "Đang tạo..." : "Tạo phương thức" }}
              </button>
...
async function onSubmit(values, { resetForm, setErrors }) {
  try {
    await PaymentService.create({ name: values.name?.trim() });

    await Swal.fire("Thành công!", "Tạo phương thức thành công!", "success");
    onReset(resetForm);
    ...
    const msg =
      ...
      "Tạo phương thức thất bại. Vui lòng thử lại.";
